...
test <- permute_data_analytics(data = ppcp_meta_dist, metric = "ppcp_sum", 
                       full_model = ppcp_PI_model, metric_plot_title = "PPCP vs. IDW Population")

permute_data_analytics <- function(data, metric, full_model, metric_plot_title){
  for(i in 1:5000){
    permuted_data <- data %>%
      select(ppcp_sum) %>%
