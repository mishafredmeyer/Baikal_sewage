...
ppcp_meta_dist <- full_join(x = ppcp, y = metadata_dist)
...
whole_fatty_acid_metaMDS <- metaMDS(comm = fatty_acid_whole_wide[ , 5:63], distance = "bray",
                                         k = 2, try = 100)
...
data_scores$Taxon <- gsub("Drapa", " Drapa spp.", data_scores$Taxon)
data_scores$Taxon <- gsub("Splash zone", "Periphyton", data_scores$Taxon)
...
nmds <- ggplot() + 
  geom_point(data=data_scores,aes(x=NMDS1,y=NMDS2, fill = Taxon), size = 10, shape = 21,
             color = "grey60", alpha = .75, stroke = 2) +  
...
## These two fatty acid groups roughly reflect Green:Diatom algal signature, which should 
## inrease with an increasing sewage signal. 
